...
export default function AddNewProduct() {
  ...
  async function handleNewElementSubmit(evt) {
    ...
    const response = await fetch(`${BASE_URL}/velvethomes/seller/addnewprod`, {
      method: "POST",
      headers: {
  ...
  return (
    <div className="cnp-body-wrapper">
      <form
        action=""
        onSubmit={handleNewElementSubmit}
        className="cnp-body-form"
      >
        ...
        <div className="cnp-wrapper">
          <div>
            <label htmlFor="cnp-subcat">
              {formData.cnpCat === "Paints"
                ? "Enter Color Of Paint : "
                : "Select The Subcategory : "}
            </label>
            {formData.cnpCat === "Tiles" && (
              <select
                name="cnpSubcat"
                value={formData.cnpSubcat}
                onChange={handleSubCatChange}
                id="cnp-subcat"
              >
                <option value="Living Room Floor Tiles">
                  Living Room Floor Tiles
                </option>
                <option value="Bed Room Floor Tiles">
                  Bed Room Floor Tiles
                </option>
                <option value="Bath Room Floor Tiles">
                  Bath Room Floor Tiles
                </option>
                <option value="Kitchen Floor Tiles">Kitchen Floor Tiles</option>
                <option value="Living Room Wall Tiles">
                  Living Room Wall Tiles
                </option>
                <option value="Bed Room Wall Tiles">Bed Room Wall Tiles</option>
                <option value="Bath Room Wall Tiles">
                  Bath Room Wall Tiles
                </option>
          ...
          {formData.cnpCat === "Paints" && (
            <input
              ...
              onChange={handleSubCatChange}
              id="snp-subcat"
              placeholder="Enter Color Of Paint"
            />
          )}
        </div>
        <div className="cnp-wrapper">
          <label htmlFor="cnp-quantity">
            Enter The Quantity of Product Available :{" "}
          </label>
          <input
            type="number"
            name="cnpQuantity"
            onChange={(e) =>
              setFormData({ ...formData, cnpQuantity: e.target.value })
            }
            value={formData.cnpQuantity}
            id="cnp-quantity"
          />
        </div>
        <div className="cnp-wrapper">
          ...
          <input
            type="number"
            name="cnpPrice"
            value={formData.cnpPrice}
            onChange={(e) =>
              setFormData({ ...formData, cnpPrice: e.target.value })
            }
            id="cnp-price"
          />
        </div>
        <div className="cnp-wrapper">
          ...
          <textarea
            ...
            cols="40"
            rows="3"
            value={formData.cnpDes}
            onChange={(e) =>
              setFormData({ ...formData, cnpDes: e.target.value })
            }
          ></textarea>
        </div>
        <div className="cnp-wrapper">
          ...
          {formData.cnpImages.map((e, ind) => (
            <div
              className="cnpTextWrapperIcon"
              style={{ display: "flex", alignItems: "center" }}
            >
              <input
                type="text"
                name="cnpImages"
                className="cnp-input-icon"
                style={{ marginRight: "0px" }}
                onChange={(evt) => handleArrayChange(evt, ind)}
                value={e}
                key={ind}
              />
              <img
                src="https://tl.vhv.rs/dpng/s/481-4811206_delete-icon-png-free-download-delete-icon-for.png"
                onClick={(e) => {
                  setFormData({
                    ...formData,
              ...
              />
              <span
                className="cnpAddNewBox"
                onClick={() => {
                  setFormData({
                    ...formData,
                    cnpImages: [...formData.cnpImages, ""],
                  });
                }}
              >
                +
        ...
        <div className="cnp-wrapper">
          <label htmlFor="cnp-keyPoints">
            Give Some Key Points About The Product :{" "}
          </label>
            ...
            >
              ...
              <span
                ...
                onClick={() => {
                  setFormData({
                    ...formData,
                    cnpKeyPoints: [...formData.cnpKeyPoints, ""],
                  });
                }}
              >
                +
        ...
        <div className="cnp-wrapper">
          <label htmlFor="">
            Enter Some features Of The Product (e.g: -
            "color","size","design-type"... )
          </label>
              ...
              return (
                ...
                >
                  <input
                    ...
                    name="cnpFeaturesKeys"
                  />{" "}
                  :
                  <input
                    type="text"
                    value={formData.cnpFeaturesValues[ind]}
                    onChange={(evt) => handleArrayChange(evt, ind)}
                    style={{ border: "1px solid black" }}
                    className="cnp-value"
                    name="cnpFeaturesKeysValues"
                  />
                  <img
                    src="https://tl.vhv.rs/dpng/s/481-4811206_delete-icon-png-free-download-delete-icon-for.png"
                    style={{ height: "27px" }}
                    onClick={(e) => {
                      setFormData({
                        ...formData,
                        cnpFeaturesKeys: formData.cnpFeaturesKeys.filter(
                          (e, index) => {
                            return index !== ind;
                          }
                        ),
                        cnpFeaturesValues: formData.cnpFeaturesValues.filter(
                          (e, index) => {
                            return index !== ind;
                          }
                        ),
                  ...
                  <span
                    ...
                    onClick={() => {
                      setFormData({
                        ...formData,
                        cnpFeaturesKeys: [...formData.cnpFeaturesKeys, ""],
                        cnpFeaturesValues: [...formData.cnpFeaturesValues, ""],
                      });
                    }}
                  >
                    +
