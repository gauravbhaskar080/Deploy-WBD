...
export default function AdminDeliveries() {
  ...
  const deliver = async (id) => {
    console.log(id);
    const response = await fetch(`${BASE_URL}/velvethomes/admin/delivered`, {
      method: "POST",
      headers: {
  ...
  const fetchData = async () => {
    setShowLoader(true);
    const response = await fetch(`${BASE_URL}/velvethomes/admin/deliveries`, {
      method: "POST",
    });
    const json = await response.json();
    if (json.success) {
    ...
    <div className="AdminHomePage">
      {showLoader ? (
        ...
        <div className="FullPage">
          <div
            className="AdminAdvertise"
            style={isScrolled ? { height: "95px" } : { height: "120px" }}
          >
            <img src={logo} alt="" />
            <div className="AdminBrandName">
              <span>Velvet Home's Customer's Page</span>
            </div>
          </div>
          <div
          ...
          >
            ...
            <div className="AdminAllCustomersMain">
              {show.length === 0 && (
                <div className="AdminAllCustomersNotFound">
              ...
              {show.map((cust, ind) => (
                ...
                >
                  ...
                  >
                    ...
                    <div className="AdminHomeCardDivValue">
                      {extractDate(new Date(cust.orderDate))}
                    </div>
                  </div>
                  <div
                    className="AdminHomeCardDiv"
                  ...
                  >
                    ...
                    <div
                      className="AdminHomeCardDivValue"
                      style={
                        check(new Date(cust.deliveryDate))
                          ? { color: "red" }
                          : { color: "#555" }
                      }
                    >
                      {extractDate(new Date(cust.deliveryDate))}
                    </div>
                  </div>
                  <div
                    className="AdminHomeCardDiv"
                  ...
                  >
                    <div
                      className="AdminAllCustomersSearchBtn"
                      style={{ marginLeft: "0", width: "100px" }}
                      onClick={() => showMoreDetails(ind)}
                    >
                      See More
                    </div>
                  </div>
      ...
      {showDelDetail && (
        ...
        >
          ...
          <div
            className="PinfoOverlayMain"
            style={{ backgroundColor: "#A6F6FF" }}
          >
            <div className="PinfoOverlayImgWrapper">
              <img
                src={showDelDetail.product.images[0]}
            ...
            <div className="PinfoOverlayMainContent">
              ...
              <div className="PinfoOverlayMainDes">
                ...
                <div className="PinfoOverlayMainDesValue">
                  ...
                  {Math.floor(
                    (showDelDetail.quantity *
                      showDelDetail.product.price *
                      (100 - showDelDetail.discount)) /
                      100
                  )}{" "}
                  /-
                </div>
              ...
              <div className="PinfoOverlayMainDes">
                ...
                <div className="PinfoOverlayMainDesValue">
                  {showDelDetail.status}
                </div>
              </div>
              {showDelDetail.status === "Pending" && (
                <div className="PinfoOverlayMainDes">
                  <div className="PinfoOverlayMainDesTitle">
              ...
              {showDelDetail.status === "Pending" && (
                <div className="PinfoOverlayBtn">
                  <div
                    className="PinfoOverlayBtnMain"
                    onClick={() => deliver(showDelDetail._id)}
                    style={{
                      display: "flex",
                      justifyContent: "center",
                      alignItems: "center",
